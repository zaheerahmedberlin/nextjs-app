...
try {
  // No ELASTICSEARCH_URL configured means there's nowhere to connect —
  // defaulting to localhost:9200 anyway made every search silently try
  // (and retry) a doomed connection, costing several seconds per
  // cache-miss request before falling back to Postgres. Skip the client
  // entirely instead so callers get an immediate null and go straight
  // to the DB path.
...
export async function searchProducts({ q, category, minPrice, maxPrice, sort, page = 1, limit = 24, inStockOnly, includeInactive, activeFrom, activeUntil }) {
  ...
  if (minPrice)              filter.push({ range: { price: { gte: parseFloat(minPrice) } } });
  filter.push({ range: { price: { gt: 0 } } }); // exclude €0 giveaway/misconfigured listings
