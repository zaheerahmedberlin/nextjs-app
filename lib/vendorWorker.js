...
async function loadCategoryMappings(vendorId) {
  const { rows } = await query(
    `SELECT vendor_category, category_id FROM vendor_category_mappings WHERE vendor_id = $1`,
    [vendorId]
  );
  // key: lowercase vendor category string → category_id
  return Object.fromEntries(rows.map((r) => [r.vendor_category.toLowerCase().trim(), r.category_id]));
}
...
async function insertBatch(rows, vendorId, vendorName) {
  ...
  // Load mappings once per batch group
  const categoryMap = await loadCategoryMappings(vendorId);

  // Fallback: sonstiges category id
  const { rows: sRows } = await query(`SELECT id FROM categories WHERE slug = 'sonstiges' LIMIT 1`);
  const sonstigesId = sRows[0]?.id ?? null;
  ...
  for (const row of rows) {
    ...
    // Resolve category_id from mapping, fall back to sonstiges
    const rawCat = (row.category || "").toLowerCase().trim();
    const categoryId = rawCat ? (categoryMap[rawCat] ?? sonstigesId) : sonstigesId;

    try {
      await query(
        `INSERT INTO products
           (external_id, title, description, image, url, ean,
            price, old_price, in_stock, vendor_id, category, category_id)
