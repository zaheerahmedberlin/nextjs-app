...
// Log all available DB-related env vars for debugging
console.log("[db] ENV keys:", Object.keys(process.env).filter(k => k.includes("PG") || k.includes("DATABASE") || k.includes("POSTGRES")).join(", "));
console.log("[db] DATABASE_URL present:", !!process.env.DATABASE_URL);

const dbUrl = process.env.DATABASE_URL || "";
console.log("[db] resolved URL present:", !!dbUrl);

const poolConfig = {
  connectionString: dbUrl || "postgresql://localhost:5432/preisgucken",
  max: 20,
